    ...
    <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-10 px-6 py-8 sm:px-12 lg:px-20">
      <header className="flex flex-col gap-4 border-b border-border pb-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="mb-2 font-body-alt text-sm font-semibold uppercase tracking-[0.12em] text-primary-active">
              {destination.region}
            </p>
            <h1 className="text-[clamp(2rem,4vw,3.5rem)] font-bold tracking-[-0.06em] text-foreground">
              {destination.title}
            </h1>
            {destination.subtitle && (
              <p className="mt-3 max-w-2xl font-body-alt text-lg leading-relaxed text-text-secondary">
                {destination.subtitle}
              </p>
            )}
          </div>
          <button
            ...
            className="shrink-0 rounded-full p-2 text-foreground transition hover:bg-background"
          >
      ...
      <motion.section
        {...fadeUp}
        className="grid gap-8 border-b border-border pb-10 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]"
      >
        <div className="flex flex-col justify-center gap-7">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-border p-4">
              <p className="text-sm text-text-secondary">Best season</p>
              <p className="mt-1 font-body-alt font-semibold text-foreground">
                {destination.best_season || "Ask our travel team"}
              </p>
            </div>
            <div className="rounded-xl border border-border p-4">
              <p className="text-sm text-text-secondary">Trips available</p>
              <p className="mt-1 font-body-alt font-semibold text-foreground">
                {packages.length} curated{" "}
                {packages.length === 1 ? "trip" : "trips"}
              </p>
            </div>
          </div>
        </div>
        {image && (
          <div className="relative min-h-[340px] overflow-hidden rounded-2xl bg-background lg:min-h-full">
            <Image
              src={image}
      ...
      <section className="flex flex-col gap-7 pb-8">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="font-body-alt text-sm font-semibold uppercase tracking-[0.12em] text-primary-active">
              Explore here
            </p>
            <h2 className="mt-1 text-2xl font-semibold tracking-[-0.04em] text-foreground">
              Packages in {destination.title}
            </h2>
          </div>
          <Link
            href={`/packages?destination=${encodeURIComponent(destination.slug)}`}
            className="font-body-alt font-semibold text-foreground underline underline-offset-4"
          >
            View all packages
          </Link>
        </div>
        {packages.length > 0 ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {packages.map((pkg) => (
              <PackageCard
                key={pkg.id}
                image={pkg.image?.src || pkg.image?.url || ""}
                title={pkg.title}
                description={pkg.summary || ""}
                price={priceLabel(pkg.currency, pkg.price)}
          ...
          <div className="rounded-2xl border border-dashed border-border p-8 font-body-alt text-text-secondary">
            Packages for this destination are being prepared. Send us an enquiry
            and we’ll help plan your trip.
          </div>
        )}
      </section>
    </div>
