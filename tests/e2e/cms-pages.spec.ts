...
async function expectNoHorizontalOverflow(
  ...
  ).toBeLessThanOrEqual(audit.viewport + 1);
}

test("package page renders CMS screens and whole-number group pricing", async ({
  page,
}) => {
  await page.goto(packagePath);

  await expect(
    page.getByRole("heading", { level: 1, name: "Journey to fish lake" }),
  ).toBeVisible();
  await expect(
...
test("blog post renders its Wagtail-managed article screens", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", {
      level: 1,
      name: "Annapurna: The Case for Going Slow",
    }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("heading", { name: /Create memories/ }),
  ).toBeVisible();
  await expectNoHorizontalOverflow(page);
});
...
test("checkout does not invent a rating", async ({ page }) => {
  ...
  await expect(
    page.getByText("UNESCO Heritage Site & Lumbini Sightseeing"),
  ).toBeVisible();
